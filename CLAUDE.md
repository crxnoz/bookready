...
# Stash any uncommitted lockfile drift before pulling. Some npm versions
# regenerate web/package-lock.json during `npm install` even when the
# lock content is already up to date; the regenerated bytes show up as
# uncommitted changes on the next deploy and block `git pull` with
# "would overwrite local changes". The regenerated file is never
# meaningful drift, so stash it (the `|| true` makes the empty-stash
# case a no-op) and drop it after the pull succeeds.
# Hit twice on 2026-06-09 across two parallel agent sessions.
git stash push -- web/package-lock.json 2>/dev/null || true

git pull origin main

# Drop the stash if one was pushed. `git stash drop` exits 1 if the
# stack is empty, which is the no-op case — silence it.
...
- **Other resources** are their own tables with full CRUD: `gallery_items`, `results_items` (+ `results_groups`). All hard-delete; hide via `is_active` toggle. (`results_items` was renamed from `before_after_items` in M3; the legacy table no longer exists.)
- **Policies + Before/After** live in their own backing tables and are returned by `PublicSiteController` from those tables ONLY — `business_policies` (one row per tenant, with `custom_groups` as a JSON column) and `results_items` + `results_groups`. The controller **does NOT** read `template_settings.settings_json.policy` or `.results` even though those keys exist in `TemplateDefaults`. Implication for seed/polish scripts: writing policy/results content into `template_settings` does nothing — the public site will render the shared section's `emptyText`. Always write to the real tables. We hit this on 2026-06-10 polishing inkhouse + clarity; the fix scripts at `tmp/{slug}-policy-results-fix.php` are the reference shape. `PoliciesEditorPanel` inside the Website hub uses the same `getEditorPolicies`/`updateEditorPolicies` API as the standalone `/editor/policies` page (which hits `business_policies`).
- **Owner-seed gotcha (mass-assignment guard)**: `App\Models\User` does NOT have `email_verified_at` or `terms_accepted_at` in `$fillable`, so `User::create(['email_verified_at' => now(), 'terms_accepted_at' => now(), ...])` **silently drops both columns** — no exception, no warning, the row just lands with both `NULL`. The result is invisible until the owner logs into the editor and hits the `EmailVerifyBanner` (every `/editor/*` page) and the publish gate (`BusinessProfileController` blocks `site_visibility='public'` with HTTP 422 `requires_verification: true`). When seeding a demo tenant or any owner user from a script, set those columns via `forceFill(...)->save()` AFTER `User::create(...)`, not in the create call. Reference: `tmp/verify-demo-owners.php`. Hit this on 2026-06-10 seeding inkhouse + clarity — the original `inkhouse-seed.php` and `clarity-seed.php` *intended* to set both timestamps and looked correct in source; we only caught it via a DB audit after the user reported the verify prompt. Same pattern likely applies to any other guarded User column added later (e.g. `is_admin`, `terms_version`).
