@extends('mail.layouts.bookready', [
  ...
  'intro'     => "Hi " . $customerName . ", we'll watch for cancellations in your date range and email you the moment a matching spot opens. The first person to claim it gets the slot — we'll send you a one-click link.",
])
...
<table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="border:1px solid rgba(18,18,18,0.08);border-collapse:collapse;">
  ...
  @include('mail.partials.kv', ['label' => 'Watching', 'value' => \Carbon\Carbon::parse($earliestDate)->format('M j') . ' – ' . \Carbon\Carbon::parse($latestDate)->format('M j, Y')])
</table>
@endsection
