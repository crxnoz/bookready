...
export default function BookingFormEditor() {
  ...
  return (
    <div className="w-full p-3 sm:p-5 md:p-6 space-y-4">
      ...
      {list.length === 0 && (
        <div className="bg-white border border-[rgba(18,18,18,0.10)] px-5 py-10 text-center">
          ...
          <button
            type="button"
            onClick={() => setEditing('new')}
            className="mt-4 inline-flex items-center gap-1.5 text-[11px] font-semibold tracking-[0.08em] uppercase bg-near-black border border-near-black text-white px-3 py-2 hover:bg-white hover:text-near-black"
          >
  ...
  return (
    <div
      className={cn(
        ...
        q.is_active ? 'border-[rgba(18,18,18,0.10)]' : 'border-[rgba(18,18,18,0.05)] opacity-60',
      )}
    >
      ...
      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-2 flex-wrap">
          <p className="text-[13px] font-semibold text-near-black truncate">{q.label}</p>
          {q.required && (
            <span className="text-[9px] font-bold tracking-[0.06em] uppercase border border-[rgba(180,40,40,0.30)] bg-[rgba(180,40,40,0.06)] text-[#b42828] px-1.5 py-0.5">
              Required
            </span>
          )}
        ...
        <p className="text-[11px] text-muted-text mt-0.5" title={scopeTitle}>
          {scopeLabel}
      ...
      >
        ...
        <div className="p-4 space-y-4">
          ...
          <div>
            ...
            <p className="text-[11px] text-muted-text mt-1.5">{TYPE_META[type].hint}</p>
          </div>
          ...
          <label className="flex items-center gap-2 cursor-pointer">
            ...
            />
            <span className="text-[12px] text-near-black">Required to book</span>
          </label>

          {/* Scope */}
          <div className="border-t border-[rgba(18,18,18,0.08)] pt-3">
            <p className="text-[10px] font-bold tracking-[0.14em] uppercase text-muted-text mb-2">Show on</p>
            <div className="flex gap-1.5 mb-2">
              <button
                type="button"
                onClick={() => setScope('all')}
                className={cn(
                  'inline-flex items-center gap-1.5 text-[11px] font-semibold px-3 py-1.5 border whitespace-nowrap',
                  scope === 'all'
            ...
            {scope === 'services' && (
              <div className="bg-white border border-[rgba(18,18,18,0.10)] max-h-48 overflow-y-auto">
                {services.length === 0 && (
                  <p className="text-[11px] text-muted-text p-3">No active services yet.</p>
                )}
                {services.map(s => {
                  ...
                  return (
                    ...
                    >
                      ...
                      />
                      <span className="text-[12px] text-near-black">{s.name}</span>
                    </label>
                  )
                })}
              </div>
  ...
  return (
    <div>
      <div className="flex items-baseline justify-between mb-1.5">
        <label className="block text-[10px] font-bold tracking-[0.14em] uppercase text-muted-text">
          Options
        </label>
        <span className="text-[10px] text-muted-text">{rows.length}/20 · need at least 2</span>
      </div>
        ...
        {rows.map((v, i) => (
          <div key={i} className="flex items-center gap-2">
            <input
              ...
              className="flex-1 bg-white border border-[rgba(18,18,18,0.15)] px-3 py-2 text-sm text-near-black placeholder:text-[#c4bcb6] focus:outline-none focus:border-near-black"
            />
            <button
              type="button"
              onClick={() => remove(i)}
