...
/**
 * Tab container. Provides the outer padding + vertical gap so the section
 * cards inside never touch the viewport edges and sit apart from each other
 * (matches the Services/Staff card layout).
 */
export function TabShell({ children, className }: { children?: React.ReactNode; className?: string }) {
  return <div className={cn('p-3 sm:p-5 md:p-6 space-y-4', className)}>{children}</div>
}

/** One-line intro under the in-page sub-nav (plain muted copy above the cards). */
export function TabIntro({ children }: { children: React.ReactNode }) {
  return <p className="text-xs text-muted-text">{children}</p>
}
